...
def register(username: str, password: str, role: str, gender:str) -> dict | None:
    from app import controller
    account: Account = controller.search_account_by_username(username)
    if account == None:
        hashed_password: str = ph.hash(password)
        if role == "customer":
            new_account: Account = controller.add_customer(username, hashed_password, gender)
        elif role == "mate":
            new_account: Account = controller.add_mate(username, hashed_password, gender)
        else:
            return None

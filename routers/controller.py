...
def add_post(body: PostModel):
    from app import controller
    post = controller.add_post(body.description, body.picture)
    if post == None:
        return res.error_response_status(status.HTTP_404_NOT_FOUND, "Error in add post")
    res.success_response_status(status.HTTP_200_OK, "Add Post Success", data=[{'account_detail' : acc.get_account_details()} for acc in mate_list])
